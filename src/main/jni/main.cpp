...
// g_UnlockAll defined in UnlockSkin.h
...
void ReadConfig() {
    ...
    if (configFile.is_open()) {
        try {
            ...
            if (j.contains("UnlockCustomSkin")) g_UnlockAll = j["UnlockCustomSkin"].get<bool>();
            ...
            if (g_DebugMode) {
                LOGI("Config Loaded: UnlockAll=%d, EnableDebugMenu=%d",
                     g_UnlockAll, g_EnableDebugMenu);
            }
...
void ApplyFeatures() {
    ...
    // --- FITUR: Unlock Skin & GM (Handled by Init functions) ---
